...
def _definition_as_dict(inspection: Any) -> dict[str, Any]:
    return {
        "entity_id": inspection.entity_id,
        "domain": inspection.domain,
        "defining_file": inspection.defining_file,
        "trigger": list(inspection.trigger),
        "condition": list(inspection.condition),
        "action": list(inspection.action),
        "references": [
            {"entity_id": ref.entity_id, "path": ref.path} for ref in inspection.references
        ],
    }


async def _get_domain_definition(
    hass: HomeAssistant, _entry_id: str, args: dict[str, Any], *, domain: str
) -> dict[str, Any]:
    """Structural trigger/condition/action view -- not the entity's state.

    New in the repair-orchestration layer: every prior investigation tool
    returns an entity's *state*, never where inside its own YAML an
    entity id is actually used. A stale-entity-reference or duplicate-
    action repair candidate needs the latter to target an exact
    structural location rather than a blind file-wide substitution.
    """
    from homeassistant.helpers import entity_registry as er

    from .domain.definition_inspection import inspect_automation, inspect_script
    from .infrastructure.source_definition_index import (
        SourceDefinitionIndex,
        async_read_config_source_files,
        parse_config_yaml,
    )
    ...
    if not unique_id:
        return {
            ...
            "entity_id": entity_id,
            "reason": "entity has no registry unique_id -- cannot locate its YAML definition",
        }

    files = await async_read_config_source_files(hass)
    index = SourceDefinitionIndex.build(files)
    domain_index = index.automation if domain == "automation" else index.script
    defining_files = domain_index.ids_to_files.get(unique_id, ())
    if not defining_files:
        return {"evidence_status": "not_found", "entity_id": entity_id, "unique_id": unique_id}
    if len(defining_files) > 1:
        return {
            "evidence_status": "ambiguous",
            "entity_id": entity_id,
            "unique_id": unique_id,
            "defining_files": list(defining_files),
        }

    defining_file = defining_files[0]
    document = parse_config_yaml(index.raw_files.get(defining_file, ""))
    if document is None:
        return {
            "evidence_status": "source_unavailable",
            "entity_id": entity_id,
            "reason": f"{defining_file} could not be parsed",
        }

    inspection = (
        inspect_automation(document, entity_id=entity_id, unique_id=unique_id, defining_file=defining_file)
    ...
    return {"evidence_status": "observed", "definition": _definition_as_dict(inspection)}


async def _get_automation_definition(
    hass: HomeAssistant, entry_id: str, args: dict[str, Any]
) -> dict[str, Any]:
    return await _get_domain_definition(hass, entry_id, args, domain="automation")


async def _get_script_definition(
    hass: HomeAssistant, entry_id: str, args: dict[str, Any]
) -> dict[str, Any]:
    return await _get_domain_definition(hass, entry_id, args, domain="script")
...
def _tools(entry_id: str) -> list[llm.Tool]:
    ...
    tools: list[llm.Tool] = [
        ...
        _InvestigationTool(
            name="hamie_get_automation_definition",
            description=(
                "Get one automation's own trigger/condition/action definition and "
                "every entity id it references, with each reference's exact "
                "structural location."
            ),
            parameters=entity_schema,
            handler=_get_automation_definition,
            entry_id=entry_id,
        ),
        _InvestigationTool(
            name="hamie_get_script_definition",
            description=(
                "Get one script's own action sequence and every entity id it "
                "references, with each reference's exact structural location."
            ),
            parameters=entity_schema,
            handler=_get_script_definition,
            entry_id=entry_id,
        ),
        _InvestigationTool(

...
if sys.version_info < (3, 12):
    # hamie/domain/serialization.py (and a few sibling domain modules) use
    # PEP 695 generic function syntax, which the Python parser itself
    # cannot read below 3.12 -- that surfaces as a bare `SyntaxError:
    # expected '('` deep inside an unrelated import, with no indication
    # it's a version problem. Fail here instead, with the actual answer,
    # before any of those modules are ever parsed: importing this package
    # always runs this check first, since Python imports a parent package
    # before any of its submodules.
    raise RuntimeError(
        "HAMIE requires Python 3.12 or newer "

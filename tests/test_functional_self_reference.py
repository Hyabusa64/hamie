...
def test_detects_version_bump_self_reference_regression() -> None:
    base = _rec("vacuum.kitchen_cleaning", unique_id="kitchen_cleaning_v1", state="unavailable")
    sibling = _rec("vacuum.kitchen_cleaning_2", unique_id="kitchen_cleaning_v2", state="cleaning")
    package = """
template:
  - binary_sensor:
      - name: Kitchen Dispatch Gate
        unique_id: kitchen_cleaning_v2
        state: "{{ is_state('vacuum.kitchen_cleaning', 'cleaning') }}"
"""
    ...
    assert "vacuum.kitchen_cleaning_2" in finding.recommendation.action


def test_detects_second_anonymized_instance_water_goal_shape() -> None:
    ...
    base = _rec("sensor.water_goal_percentage", unique_id="water_goal_pct_r1", state="unavailable")
    sibling = _rec("sensor.water_goal_percentage_2", unique_id="water_goal_pct_r2", state="42")
    package = """
template:
  - sensor:
      - name: Water Usage Alert
        unique_id: water_goal_pct_r2
        state: "{{ states('sensor.water_goal_percentage') }}"
"""
...
def test_no_finding_when_version_tokens_do_not_indicate_a_bump() -> None:
    ...
    base = _rec("vacuum.kitchen_cleaning", unique_id="kitchen_cleaning_v2", state="unavailable")
    sibling = _rec("vacuum.kitchen_cleaning_2", unique_id="kitchen_cleaning_v1", state="cleaning")
    package = """
template:
  - binary_sensor:
      - unique_id: kitchen_cleaning_v1
        state: "{{ is_state('vacuum.kitchen_cleaning', 'cleaning') }}"
"""
    outcome = _analyze((base, sibling), _index("packages/kitchen.yaml", package))
    assert outcome.findings == ()
...
def test_uncovered_without_raw_files() -> None:
    ...
    assert outcome.findings == ()
    assert "vacuum.kitchen_cleaning" in outcome.uncovered_subjects
...
def test_malformed_group_degrades_without_aborting_scan() -> None:
    ...
    sibling_good_base = _rec(
        "vacuum.kitchen_cleaning_2", unique_id="kitchen_cleaning_v2", state="cleaning"
    )
